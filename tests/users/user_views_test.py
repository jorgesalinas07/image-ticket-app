...
def test_login_endpoint_should_return_200_status_when_login_successful(client, user):
    response = client.post(f'{endpoint}/login/', {'username': 'testuser', 'password': 'test_pass'})
...
def test_login_endpoint_should_return_404_status_when_password_is_invalid(client, user):
    response = client.post(f'{endpoint}/login/', {'username': 'testuser', 'password': 'invalid_pass'})
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'error': 'Invalid password'}

@pytest.mark.django_db
def test_signup_endpoint_should_return_201_status_when_user_creation_was_successful(client):
