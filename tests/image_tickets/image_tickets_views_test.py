...
def test_get_paginated_tickets_endpoint_should_return_200_status_when_tickets_found(auth_client):
...
def test_get_paginated_tickets_should_return_tickets_in_timeframe_when_date_filters_are_provided(
    ...
    response = client.get(f'{endpoint}', {'start_date': start_date, 'end_date': end_date})
    assert response.status_code == 200
    assert len(response.data) == 1

def test_get_paginated_ticket_should_return_403_status_when_user_not_authenticated(client):
